...
async def backtest(symbol: str = "NIFTY", bars: int = Query(2000, ge=300)) -> BacktestResponse:
    ...
    engine = BacktestEngine(RiskManager(RISK), ExecutionCostModel(COSTS), lot_size=COSTS["lot_size_nifty"])
...
async def costs(entry: float, exit: float, lots: int = 1, direction: int = 1) -> CostSummaryResponse:
    model = ExecutionCostModel(COSTS)
    ticket = ExecutionCostModel.TradeTicket(
        direction=direction,
        entry_price=entry,
        exit_price=exit,
        lots=lots,
    ...
    c = model.calculate_costs(ticket)
    ...
    return CostSummaryResponse(
        entry_cost=round(c.entry_cost, 4),
        exit_cost=round(c.exit_cost, 4),
        total_cost=round(c.total_cost, 4),
        gross_pnl=round(gross, 4),
        net_pnl=round(net, 4),
        breakeven_move=round(c.breakeven_move_per_unit, 4),
    )


@router.get("/risk-summary")
async def risk_summary() -> Dict[str, Any]:
    return RiskManager(RISK).summary()
