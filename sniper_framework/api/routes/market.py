...
from execution.broker_interface import OrderRequest, OrderSide, OrderType, TimeInForce
...
@router.post("/paper/orders", response_model=PaperOrderResponse)
async def submit_paper_order(request: PaperOrderRequest) -> PaperOrderResponse:
    try:
...
@router.get("/paper/positions", response_model=List[PositionResponse])
async def paper_positions() -> List[PositionResponse]:
    return [
        PositionResponse(
            symbol=p.symbol,
            qty=p.qty,
        ...
        for p in PAPER_BROKER.get_positions().values()
    ]
